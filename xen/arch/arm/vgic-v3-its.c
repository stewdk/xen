...
{
    struct pci_host_bridge *bridge = pci_find_host_bridge(PCI_SEG(host_devid),
                                                          PCI_BUS(host_devid));
...
static int its_handle_mapd(struct virt_its *its, uint64_t *cmdptr)
{
    ...
    if ( !is_hardware_domain(its->d) )
        host_doorbell_address = its_get_host_doorbell(its, host_devid);
    else
...
{
    int ret;
    static unsigned int devid_bits = 0;
    static unsigned int evid_bits = 0;
