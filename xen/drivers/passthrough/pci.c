...
static void __init cf_check reserve_bar_range(struct pci_dev *pdev, uint8_t reg,
                                              bool is_64bit, uint64_t addr,
                                              uint64_t size)
{
    if ( pci_check_bar(pdev, maddr_to_mfn(addr),
                       maddr_to_mfn(addr + size - 1)) )
        pci_reserve_bar_range(pdev, addr, size);
}
                                             ...
                                             uint64_t size)
{
    if ( !pci_check_bar(pdev, maddr_to_mfn(addr),
                        maddr_to_mfn(addr + size - 1)) )
    {
        addr = pci_get_new_bar_addr(pdev, size);
...
static int __init cf_check bars_iterate(struct pci_seg *pseg, void *arg)
{
    ...
    uint64_t addr, size;
    void (*cb)(struct pci_dev *, uint8_t, bool, uint64_t, uint64_t) = arg;
